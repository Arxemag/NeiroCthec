...
<body>
  ...
  <div class="card">
    ...
    <div class="muted">Читает/сохраняет через `/books/settings/audio` для текущего `X-User-Id` (user config хранится в БД `user_audio_configs` и мерджится с global `audio.yaml`, поэтому `engine.language`/`voices` не теряются).</div>
    <div class="grid2">
